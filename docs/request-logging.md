...
- **c→g**：`build_redacted_envelope()`（`crates/server/src/ingress/observability.rs`）
...
- **g→p**：egress header 以 **reqwest 实际构建的请求为准**。在每个上游
  ...
  `finalize_egress()`（`ingress/observability.rs`）执行 `builder.build()` 得到
...
例外：`x-request-id` 会被**改写覆盖**为网关为该请求生成的 request id（即
